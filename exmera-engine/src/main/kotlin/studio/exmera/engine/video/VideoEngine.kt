...
import studio.exmera.engine.imaging.MotionVector
import studio.exmera.engine.imaging.RgbImage
import kotlin.math.abs
import kotlin.math.max
import kotlin.math.min

/** Phase 15: bounded temporal video processing primitives. */
...
data class VideoProcessConfig(val temporalWindow: Int = 5, val denoiseStrength: Float = .35f, val stabilization: Float = .65f) {
    init { require(temporalWindow in 2..16); require(denoiseStrength in 0f..1f); require(stabilization in 0f..1f) }
}
interface OpticalFlowBackend { fun estimate(previous: RgbImage, current: RgbImage): OpticalFlowField }

/** Fast CPU translation-flow baseline. The interface is intentionally replaceable by dense EXM flow. */
class CpuOpticalFlowBackend(private val radius: Int = 6) : OpticalFlowBackend {
    override fun estimate(previous: RgbImage, current: RgbImage): OpticalFlowField {
        ...
        for (dy in -radius..radius) for (dx in -radius..radius) {
            var error = 0f; var n = 0
            for (y in radius until max(radius + 1, h - radius) step max(1, h / 24)) for (x in radius until max(radius + 1, w - radius) step max(1, w / 24)) {
                val xx = (x + dx).coerceIn(0, w - 1); val yy = (y + dy).coerceIn(0, h - 1)
                error += abs(luma(previous, x, y) - luma(current, xx, yy)); n++
            }
...
class TemporalFrameBuffer<T>(private val capacity: Int) : AutoCloseable where T : AutoCloseable {
...
object TemporalDenoiser {
    fun process(frames: List<VideoFrame>, strength: Float): RgbImage {
        require(frames.isNotEmpty()); val ref = frames.last().image; val w=ref.width; val h=ref.height
        if (frames.size == 1 || strength <= 0f) return RgbImage(w,h,ref.pixels.copyOf())
        val out=FloatArray(w*h*3); val alpha=strength.coerceIn(0f,1f)
        for (p in 0 until w*h) for (c in 0..2) { var mean=0f; for (f in frames) mean += f.image.pixels[p*3+c]; mean/=frames.size; val cur=ref.pixels[p*3+c]; out[p*3+c]=(cur*(1f-alpha)+mean*alpha).coerceIn(0f,1f) }
...
object Stabilizer {
    fun estimate(flow: OpticalFlowField, strength: Float): StabilizationTransform {
        var dx=0f; var dy=0f; var n=0
        for (p in 0 until flow.width*flow.height) { dx += flow.vectors[p*2]; dy += flow.vectors[p*2+1]; n++ }
        val scale=strength.coerceIn(0f,1f)/max(1,n)
        return StabilizationTransform(-dx*scale,-dy*scale,flow.confidence)
    }
}
...
class VideoEngine(private val flowBackend: OpticalFlowBackend = CpuOpticalFlowBackend(), private val config: VideoProcessConfig = VideoProcessConfig()) {
    ...
    fun process(frames: List<VideoFrame>): RgbImage = TemporalDenoiser.process(frames.takeLast(config.temporalWindow), config.denoiseStrength)
